...
class TrieNode {
 public:
  ...
  TrieNode(TrieNode &&other_trie_node) noexcept {
    ...
    for(auto& item:other_trie_node.children_){
      children_[item.first] = std::move(item.second);
    }
  }
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for(unsigned long  i=0;i<key.size();i++){
      auto ne = (*cur)->GetChildNode(key[i]);
      ...
      if(i!=key.size()-1){
        ...
        if(ne==nullptr)
           (*cur)->InsertChildNode(key[i],std::make_unique<TrieNodeWithValue<T>>(key[i],value));
        else if(!(*ne)->IsEndNode()){
            auto  tmp = (*cur)->GetChildNode(key[i]);
            auto trv = std::make_unique<TrieNodeWithValue<T>>(std::move(**tmp),value);
            (*cur)->RemoveChildNode(key[i]);
            (*cur)->InsertChildNode(key[i],std::move(trv));
        }
        else{
          latch_.WUnlock();
          return false;
  ...
  bool Remove(const std::string &key) { 
    ...
    latch_.WLock();
    auto cur = &root_;
    std::vector<std::unique_ptr<TrieNode>*> route;
    ...
    for(long i=key.size()-1;i>=0;i--){
      std::unique_ptr<TrieNode>* parent = i==0?&root_:route[i-1];
      if(!(*route[i])->HasChildren()){
        (*parent)->RemoveChildNode(key[i]);
      }
      else
        break;
    }
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    *success = true;
    auto cur = &root_;
    unsigned long i;
    for(i=0;i<key.size()&&cur!=nullptr;i++){
      cur  = (*cur)->GetChildNode(key[i]);
    }
    if(i<key.size()||cur==nullptr||!(*cur)->IsEndNode())
      *success = false;
    else{
      auto node = dynamic_cast<TrieNodeWithValue<T>*>((*cur).get());
      if(node==nullptr)
        *success = false;
      auto ret = node->GetValue();
      if(*success){
        latch_.RUnlock();
        return ret;
      }
